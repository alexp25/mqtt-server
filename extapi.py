...
class ExtApi(Thread):
            ...
            if e["ENABLED"]:
                ...
                for dbs in db:
                    if dbs["sensor_id"] == sd["FULL_ID"]:
                        ...
                        break
                if not exists:
                    s = self.create_sensor_model(sd["ID"], sd["TYPE"], sd["NC"], sd["LOG_RATE"], sd["TOPIC"], None)
    ...
    def request_data(self):
        for e in self.ext_apis:
            if e["ENABLED"]:
                ...
                print(out)
                sd = e["SENSOR"]
                s = self.create_sensor_model(sd["FULL_ID"], sd["TYPE"], sd["NC"], sd["LOG_RATE"], sd["TOPIC"], None)
    ...
    def create_sensor_model(self, id, type, nc, log_rate, topic, data):
        s1 = Sensor()
